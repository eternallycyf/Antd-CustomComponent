class PubSub {
  ...
  emit(eventType: string, ...args: any[]) {
    if (!this.handles.hasOwnProperty(eventType)) throw new Error('eventType is not exist');
  ...
  off(eventType: string, handle: any) {
    if (!this.handles.hasOwnProperty(eventType)) throw new Error('eventType is not exist');
    if (typeof handle !== 'function') throw new Error('handle must be a function');
    this.handles[eventType].forEach((item: any, key: number, arr: any[]) => item === handle && arr.splice(key, 1));
    return this;
  }
}
